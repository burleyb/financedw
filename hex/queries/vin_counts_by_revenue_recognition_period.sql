-- VIN Counts by Revenue Recognition Period
-- Summary query showing count of VINs by revenue recognition period
...
-- Summary: Count of VINs by revenue recognition period
SELECT 
    frr.revenue_recognition_period,
    frr.revenue_recognition_year,
    frr.revenue_recognition_month,
    frr.revenue_recognition_quarter,
    COUNT(DISTINCT dv.vin) as vin_count,
    COUNT(DISTINCT dv.deal_id) as deal_count,
    MIN(frr.revenue_recognition_date_utc) as earliest_recognition_date,
    MAX(frr.revenue_recognition_date_utc) as latest_recognition_date
