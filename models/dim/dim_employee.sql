...
CREATE TABLE IF NOT EXISTS gold.finance.dim_employee (
  employee_key BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1),
  employee_id_source STRING NOT NULL, -- Natural key from source system (e.g., BambooHR ID)
  first_name STRING,
  last_name STRING,
  full_name STRING,
  job_title STRING,
...
-- ALTER TABLE gold.finance.dim_employee ADD CONSTRAINT dim_employee_id_unique UNIQUE (employee_id_source);
...
USING (
  ...
  SELECT
    CAST(id AS STRING) AS employee_id_source,
    first_name,
    last_name,
  ...
  -- QUALIFY ROW_NUMBER() OVER (PARTITION BY id ORDER BY lastChanged DESC) = 1
) AS source
-- Match on the natural key (source system ID)
ON target.employee_id_source = source.employee_id_source
...
-- Insert new employees (employee_key is auto-generated)
