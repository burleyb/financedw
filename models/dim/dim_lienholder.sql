...
-- Update existing lienholders if name changes (unlikely if key is slug, but possible if key is name)
WHEN MATCHED AND (
    COALESCE(target.lienholder_name, '') <> COALESCE(source.lienholder_name, '')
  ) THEN
  UPDATE SET
...
WHEN NOT MATCHED THEN
  INSERT (lienholder_key, lienholder_name, _source_table, _load_timestamp)
  VALUES (source.lienholder_key, source.lienholder_name, source._source_table, CURRENT_TIMESTAMP());

-- Ensure 'Unknown' lienholder exists
MERGE INTO gold.finance.dim_lienholder AS target
USING (SELECT 'Unknown' as lienholder_key, 'Unknown' as lienholder_name, 'static' as _source_table) AS source
ON target.lienholder_key = source.lienholder_key
WHEN NOT MATCHED THEN INSERT (lienholder_key, lienholder_name, _source_table, _load_timestamp)
VALUES (source.lienholder_key, source.lienholder_name, source._source_table, current_timestamp()); 