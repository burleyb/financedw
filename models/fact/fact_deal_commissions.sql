...
CREATE TABLE IF NOT EXISTS finance_gold.finance.fact_deal_commissions (
  -- Keys
...
MERGE INTO finance_gold.finance.fact_deal_commissions AS target
USING (
  SELECT
    sbd.id AS deal_key,
    -- Fetch setter and closer from bronze.deals
    COALESCE(bld.setter_id, 'Unknown') AS employee_setter_key,
    COALESCE(bld.closer_id, 'Unknown') AS employee_closer_key,
    ...
    CAST(sbd.setter_commission * 100 AS BIGINT) AS setter_commission_amount,
    CAST(sbd.closer_commission * 100 AS BIGINT) AS closer_commission_amount,
    CAST(sbd.commissionable_gross_revenue * 100 AS BIGINT) AS commissionable_gross_revenue_amount,
  ...
  WHERE (sbd.setter_commission IS NOT NULL OR sbd.closer_commission IS NOT NULL OR sbd.commissionable_gross_revenue IS NOT NULL)
  -- Optional: Add time-based filter for incremental loads
  -- AND sbd.state_asof_utc > (SELECT MAX(_load_timestamp) FROM finance_gold.finance.fact_deal_commissions WHERE _load_timestamp IS NOT NULL)
